...
module.exports.onLocalLogin = function(req, res, next) {
  ...
  passport.authenticate('local-with-otp')(req, res, function(err) {
    if (req.xhr) {
      ...
      if (__guard__(__guard__(err != null ? err.response : undefined, x1 => x1.body), x => x.error_description) === 'invalid email or password') {
  ...
  request
    ...
    .send({
      ...
      if (err && (err.message === 'Email is invalid.')) {
        const suggestion = __guard__(Mailcheck.run({ email: req.body.email }), x => x.full);
        ...
        msg = __guard__(err.response != null ? err.response.body : undefined, x1 => x1.error) || __guard__(err.response != null ? err.response.body : undefined, x2 => x2.message) || err.message;
...
module.exports.afterSocialAuth = provider => (function(req, res, next) {
  ...
  const providerName = _s.capitalize(provider);
  const linkingAccount = (req.user != null);
  passport.authenticate(provider)(req, res, function(err) {
    let msg;
    if (__guard__(__guard__(err != null ? err.response : undefined, x1 => x1.body), x => x.error) === 'User Already Exists') {
    ...
    } else if (__guard__(__guard__(err != null ? err.response : undefined, x3 => x3.body), x2 => x2.error) === 'Another Account Already Linked') {
    ...
    } else if (__guard__(err != null ? err.message : undefined, x4 => x4.match('Unauthorized source IP address'))) {
